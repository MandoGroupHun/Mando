    ...
    public class InviteService : IInviteService
    {
        ...
        public async Task<Result<bool>> AddInvite(Invite newInvite)
        {
            try
            {
                ...
                const string huSubject = "Meghívó a Mando alkalmazásba";

                var buildingName = _dbContext.Buildings.First().HUName;

                var body = await new RazorRenderer().ParseAsync(huTemplate, new { Email = newInvite.Email, BuildingName = buildingName, Link = $"https://{_url}/Identity/Account/Register?inviteId={newInvite.InviteId}" });
